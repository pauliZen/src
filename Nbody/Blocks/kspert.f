...
*
      INCLUDE 'common2.h'
      REAL*8  XI(6),VI(6),FP(6),FD(6),XREL(3),VREL(3),
     &        ACC(3),DER(3)
*
*
...
*       Form the relative perturbing force and first derivative.
   ...
   30 CONTINUE
*
*       Check Post-Newtonian option.
      IF (KZ(11).EQ.0) GO TO 60
*
      DO 40 K = 1,3
          XREL(K) = XI(K) - XI(K+3)
          VREL(K) = VI(K) - VI(K+3)
   40 CONTINUE
*
*       Obtain PN acceleration and derivative.
      DT = STEP(I1)
      CALL PNPERT(BODY(1),BODY(2),XREL,VREL,ACC,DER,DT)
*
*       Combine PN acceleration and dynamical perturbation for GAMMA(IPAIR).
      DO 50 K = 1,3
          FP(K) = FP(K) + ACC(K)
          FD(K) = FD(K) + DER(K)
   50 CONTINUE
