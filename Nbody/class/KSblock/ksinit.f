...
*
      FAC1 = 1.0/R(IPAIR)
      FAC2 = -TDOT2(IPAIR)*FAC1**3
      FAC3 = FAC1**2
      DO 50 K = 1,4
          UKS(K,IPAIR) = U(K,IPAIR)
          UKSD(K,IPAIR) = FAC1*UDOT(K,IPAIR)
          UKSD2(K,IPAIR) = FAC2*UDOT(K,IPAIR) + FAC3*2.0*FU(K,IPAIR)
   50 CONTINUE
*
      DO 60 K = 5,8
          UKS(K,IPAIR) = UDOT(K-4,IPAIR)
          UKSD(K,IPAIR) = FAC1*2.0*FU(K-4,IPAIR)
          UKSD2(K,IPAIR) = FAC2*2.0*FU(K-4,IPAIR) +
     &                     FAC3*6.0*FUDOT(K-4,IPAIR)
   60 CONTINUE
      UKS(9,IPAIR) = H(IPAIR)
      UKSD(9,IPAIR) = FAC1*HDOT(IPAIR)
      UKSD2(9,IPAIR) = FAC2*HDOT(IPAIR) + FAC3*HDOT2(IPAIR)
*
      DO 70 K = 1,9
          UKSD3(K,IPAIR) = 0.0
          UKSD4(K,IPAIR) = 0.0
