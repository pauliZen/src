         ...
         eval=0.0
         open(66,file='xyz') !time and coords will be written -> xyz file
         read(5,*)tmx,tincr,eps,icm 
c                   tmx=max time
...
c                                icm= cm-index (>0=>reduce2cm)
         ...
         k1=i*3-2
         k2=k1+2
         read(5,*)m(i),(xr(k),k=k1,k2),(vr(k),k=k1,k2) 
           ! read mass & initial coord/vels
         end do
         nb=3
         newcase=1
         time=0
...
123      format(1x,f12.4,1p,9g14.6)
        ...
        sm=sm+m(i)
        do k=1,3
        cm(k)=cm(k)+m(i)*x(k+3*(i-1)) ! compute cm
        end do
        end do
        do k=1,3
        cm(k)=cm(k)/sm
        ...
        do i=1,3
        do k=1,3
        x(k+3*(i-1))=x(k+3*(i-1))-cm(k) ! cm coordinates
        end do
        end do
        ...
        return
         ...
         subroutine ThreeB(mr,time,xr,vr,tincr,newcase,eps) ! integrate 3B motion 
         implicit real*8 (a-h,m,o-z)
...
c
         call  DIFSYAB(NVX,EPS,SY,h,dummy,Y) ! Burlisch-Stoer integrator 
         ...
         do i=1,3
         i0=3*i-3
         do k=1,3 ! add cm-motion contributions
...
c        take -sum of two shorter ones for the longest
         ...
         subroutine SubSteps(Y,H,Leaps) ! substeps of size H/Leaps.
         implicit real*8 (a-h,o-z)
         real*8 y(20)
         save
         ...
         Te=Tk+v(0)
         dt=s/Te                 ! time increment (this s/Te regularizes the algorithm)
         x(0)=x(0)+dt            ! add it 
         do k=1,9
         ...
         dt=s/U ! time increment in terms of potential (not used 4 time)
         do k=1,3
         ...
         function cdot(a,b) ! scalar product
         implicit real*8 (a-h,m,o-z)
         real*8 a(3),b(3)
        ...
        SUBROUTINE DIFSYAB(N,EPS,S,h,t,Y) ! a BS integrator
        implicit real*8 (a-h,o-z)
c       Bulirsch-Stoer Extrapolator. Original by Bulirsch. Modif s.m. 
        parameter (NMX=20,NMX2=2*NMX,nmx27=nmx2*7) ! NMX=MAX(N),N=3*NB
     ...
     +  ,DT(NMX2,7),D(7),S(N),EP(4)
        ...
        IF(N.gt.NMX)write(6,*) ' too many variables!',char(7),'MAX=',NMX
        JTI=0
        FY=1
        redu=0.7d0
10      tN=t+H
