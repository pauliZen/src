...
*
     ...
     &                TCL,STEPCL,NCL,NEWCL
      COMMON/ECHAIN/  ECH
      REAL*8  X1(3,4),V1(3,4),UI(4),VI(4),XREL2(3),VREL2(3),AS(20)
      REAL*4  XS(3,NMAX),VS(3,NMAX),BODYS(NMAX)
...
*     --11/11/12 12:52-lwang-output-to-root-----------------------------*
***** Note:change out3 to root-----------------------------------------**
***** --Original:------------------------------------------------------**
c$$$      IF (FIRST) THEN
...
*     CLOSE (UNIT=3)
***** --root:----------------------------------------------------------**
      call bdata_header(ntot,model,nrun,as)
      do 88 j=1, NTOT
         call bdata_body(bodys(j),xs(1,j),xs(2,j),xs(3,j),vs(1,j),
     &        vs(2,j),vs(3,j),name(j),kstar(j))
 88   continue
      call bdata_write()
*     --11/11/12 12:52-lwang-end-output-to-root-------------------------*
*
