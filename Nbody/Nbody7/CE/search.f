...
*
      J = LIST(L,I)
*     --11/13/12 16:18-lwang-suppress p-p ks----------------------------*
***** Note:------------------------------------------------------------**
*     IF (STEP(J).GT.SMIN) GO TO 2      
*     --11/13/12 16:19-lwang-end----------------------------------------*
      A1 = X(1,J) - X(1,I)
      A2 = X(2,J) - X(2,I)
      A3 = X(3,J) - X(3,I)
      RIJ2 = A1*A1 + A2*A2 + A3*A3
*     --11/13/12 16:31-lwang-change-------------------------------------*
***** Note:change rmin22 to 10*rmin22----------------------------------**
      IF (RIJ2.GT.10*RMIN22) GO TO 2      
*     --11/13/12 16:31-lwang-end-change---------------------------------*
*
      FIJ = BODY(J)/RIJ2
...
*       Also check possible c.m. body of chain regularization (NAME = 0).
      ...
      END IF
*     --11/13/12 16:20-lwang-suppress p-p ks----------------------------*
***** Note:------------------------------------------------------------**
      if (body(I)+body(JCOMP).LT.2E-5.AND.RIJMIN.GT.0.1*RMIN) then
         go to 10
      end if
*     --11/13/12 16:20-lwang-end----------------------------------------*
*      
