...
*             call flush(6)
             ...
             WL_A(3)=A3
             if (i.GT.N) then
                WL_LM(1)=body(2*(i-N)-1)
                WL_LM(2)=body(2*(i-N))
                WL_LM(3)=body(k)
                call storece(name(2*(i-N)-1),name(2*(i-N)),name(k),0,
     &               WL_LM,WL_A,dv,Time)
             else
                if (abs(name(i)-name(k)).NE.1) then
                   WL_LM(1)=body(i)
                   WL_LM(2)=body(k)
                   call storece(name(i),name(k),0,0,WL_LM,WL_A,dv,Time)
                end if
             end if
          end if
...
*       Sum over regularized c.m. neighbours.
      DO 50 L = NNB3,NNB1
          ...
          DV(3) = XDOT(3,K) - XIDOT(3)
          RIJ2 = A1*A1 + A2*A2 + A3*A3
*     --10/30/12 14:43-lwang-CE-single-c.m.-----------------------------*
***** Note:single particle close encounter-----------------------------**
          if (RIJ2.LT.WL_RADIUS) then
*             write (6,*) 'lW_CE: time in f :', Time
*             call flush(6)
             ...
             if (i.GT.N) then
                ...
                WL_LM(4)=body(2*(k-N))
                call storece(name(2*(i-N)-1),name(2*(i-N)),
     &               name(2*(k-N)-1),name(2*(k-N)),WL_LM,WL_A,dv,Time)
             else
                WL_LM(1)=body(i)
                WL_LM(2)=body(2*(k-N)-1)
                WL_LM(3)=body(2*(k-N))
                call storece(name(i),name(2*(k-N)-1),name(2*(k-N)),0,
     &               WL_LM,WL_A,dv,Time)
             end if
          end if
*     --10/30/12 14:43-lwang-end-CE-------------------------------------*
...
*       Sum over individual components of pair #J.
   45     A1 = X(1,K) - XI(1)
          ...
          DV(3) = XDOT(3,K) - XIDOT(3)
          RIJ2 = A1*A1 + A2*A2 + A3*A3
*     --10/30/12 14:43-lwang-no-needed=======---------------------------*
***** Note:single particle close encounter-----------------------------**
*     --10/30/12 14:43-lwang-end-CE-------------------------------------*
*
